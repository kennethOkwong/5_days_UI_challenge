import 'package:flutter/material.dart';

const colorBlack = Color.fromRGBO(48, 47, 48, 1.0);
const colorGrey = Color(0xFFD1CDCD);
const colorWhite = Color.fromARGB(255, 255, 255, 255);
const colorDarkBlue = Color.fromRGBO(20, 25, 45, 1.0);

const TextTheme textThemeDefault = TextTheme(
  displayLarge:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 26),
  displayMedium:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 22),
  displaySmall:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 20),
  headlineLarge:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 16),
  headlineMedium:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 14),
  headlineSmall:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 12),
  bodyLarge: TextStyle(
      color: colorBlack,
      fontWeight: FontWeight.w500,
      fontSize: 14,
      height: 1.5),
  bodyMedium: TextStyle(
    color: colorGrey,
    fontWeight: FontWeight.w500,
    fontSize: 14,
    height: 1.5,
  ),
  titleLarge:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w500, fontSize: 12),
  titleMedium:
      TextStyle(color: colorGrey, fontWeight: FontWeight.w500, fontSize: 12),
);

const TextTheme textThemeSmall = TextTheme(
  displayLarge:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 22),
  displayMedium:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 20),
  displaySmall:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 18),
  headlineLarge:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 14),
  headlineMedium:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 12),
  headlineSmall:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w700, fontSize: 10),
  bodyLarge: TextStyle(
      color: colorBlack,
      fontWeight: FontWeight.w500,
      fontSize: 12,
      height: 1.5),
  bodyMedium: TextStyle(
    color: colorGrey,
    fontWeight: FontWeight.w500,
    fontSize: 12,
    height: 1.5,
  ),
  titleLarge:
      TextStyle(color: colorBlack, fontWeight: FontWeight.w500, fontSize: 10),
  titleMedium:
      TextStyle(color: colorGrey, fontWeight: FontWeight.w500, fontSize: 10),
);

double padding = 25;
EdgeInsets sidePadding = EdgeInsets.symmetric(horizontal: padding);
